...
function crearComentario(){
    var fecha = new Date();
    var fecha_formateada = '' + fecha.getFullYear() + '-' +fecha.getMonth() + '-' + fecha.getDate() 
...
// Y le estamos pasando por parametro un array de palabras prohibidas, extraidas de la bd y pasadas con twig con json_encode
function censurar(palabras_ban){
    ...
    for(i=0; i<palabras_ban.length; i++){
        if(mensaje.includes(palabras_ban[i])){
            mensaje = mensaje.replace(palabras_ban[i],"*".repeat(palabras_ban[i].length))
        }
    }

    // Reemplazamos el mensaje original por el mensaje censurado
    document.getElementById("msg").value = mensaje;
}


function comprobar_textarea() {
